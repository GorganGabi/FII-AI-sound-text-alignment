...
def WikiDocumentSentences(out, id, title, tags, text):
    ...
    # header = header.lower()
    # header = re.sub("[^a-zA-Zăîâșț ]", '', header)
    #
    # print(header, file=out)
    for line in compact(text, structure=False):
        line = line.lower()
        line = re.sub("[^a-zA-Z0-9ăîâșț ]", '', line)

        newLine = ""
        for word in line.split(" "):
            pattern = re.compile("[0-9]+")
            if pattern.match(word):
                word = re.findall("\d+", word)[0]
                newLine = newLine + num2words(int(word)) + " "
            else:
...
def num2words(num):
	nums_20_90 =	['douăzeci','treizeci','patruzeci','cincizeci','șaizeci','șaptezeci','optzeci','nouăzeci']
	nums_0_19 = ['zero','unu','două','trei','patru','cinci','șase','șapte','opt',"nouă", 'zece','unsprezece','doisprezece','treisprezece','paisprezece','cincisprezece','șaisprezece','șaptesprezece','optsprezece','nouăsprezece']
	nums_dict = {100: 'sute',1000:'mii', 1000000:'milioane', 1000000000:'bilioane'}
	if num < 20:
	...
	return num2words(int(num/maxkey)) + ' ' + nums_dict[maxkey] + ('' if num%maxkey == 0 else ' ' + num2words(int(num%maxkey)))
...
def main():
    ...
    if 'bzip2' in ftypes:
        ...
        with open(args.infn, encoding='utf-8') as infile:
